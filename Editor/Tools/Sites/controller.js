...
var mainController = {
  ...
  $drop$page$language : function(dragged,dropped) {
    hui.ui.request({
      ...
      $success : function() {
        hui.ui.msg.success({text:{en:'The language is changed', da:'Sproget er ændret'}});
        list.refresh();
        selectionSource.refresh();
      }
    });
  ...
  $click$createPage : function() {
    ...
    if (template===null) {
      newPageWizard.goToStep(0);
      hui.ui.msg.fail({text:{en:'No type is selected',da:'Der er ikke valgt en type'}});
    } else if (design===null) {
      newPageWizard.goToStep(1);
      hui.ui.msg.fail({text:{en:'No design is selected',da:'Der er ikke valgt et design'}});
    } else if (frame===null) {
      newPageWizard.goToStep(2);
      hui.ui.msg.fail({text:{en:'No setup is selected',da:'Der er ikke valgt en opsætning'}});
    } else if (form.title=='') {
      newPageWizard.goToStep(4);
      hui.ui.msg.fail({text:{en:'No title is provided', da:'Der er ikke udfyldt en titel'}});
      newPageTitle.focus();
    } else {
