hui.ui.listen({
  ...
  $submit$eventFormula : function() {
    ...
    if (data.startdate) {
      ...
      hui.ui.msg.fail({text:{en:'The start date is required',da:'Startdatoen skal udfyldes'}});
      eventFormula.focus();
      return;
    }
    if (data.enddate) {
      ...
      hui.ui.msg.fail({text:{en:'The end date is required',da:'Slutdatoen skal udfyldes'}});
      eventFormula.focus();
      return;
    }
    if (data.calendars.length<1) {
      hui.ui.msg.fail({text:{en:'At least one kalendar must be selected',da:'Mindst een kalender skal vælges'}});
      eventFormula.focus();
      return;
    }
