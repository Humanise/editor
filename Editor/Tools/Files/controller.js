hui.ui.listen({
  ...
  $uploadDidFail$file : function() {
    hui.ui.msg.fail({text:{en:'Unable to add file. It may be too large.',da:'Det lykkedes ikke at filføje filen. Den er måske for stor.'}});
  },
  ...
  $click$fetchFile : function() {
    ...
    hui.ui.msg({text:{en:'Fetching file...',da:'Henter fil...'},busy:true});
  ...
  $success$fileFetched : function(data) {
    if (data.success) {
      fetchFormula.reset();
      hui.ui.msg.success({text:{da:'The file has been fetched',da:'Filen er hentet'}});
      filesSource.refresh();
      groupSource.refresh();
      typesSource.refresh();
    } else {
  ...
  $success$fileDeleted : function(response) {
    ...
    groupSource.refresh();
    typesSource.refresh();
    hui.ui.msg.success({text:{en:'The file has been deleted',da:'Filen er nu slettet'}});
  },
  $click$view : function() {
    var obj = list.getFirstSelection();
