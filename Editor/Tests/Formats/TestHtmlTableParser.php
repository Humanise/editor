...
class TestHtmlTableParser extends UnitTestCase {
  ...
  function testParseUsingHeader() {
    ...
    $this->assertEqual($firstRow['Hjemmehold'],'Dybvad Håndbold Flauenskjold IF');

    $secondRow = $table[1];
    $this->assertEqual($secondRow['Udehold'],'Bindslev/Tversted IF');
  }
}