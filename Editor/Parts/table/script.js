var partController = {
  ...
  $select$tableMenu : function(value) {
    if (value === 'deleteRow') {
      var tr = hui.closest('tr', this.selectedCell);
      if (tr) {
        hui.dom.remove(tr);
      }
...
hui.table = {
  getRowCount : function(table,part) {
    var head = hui.find(part, table);
    var headCount = 0;
    if (head) {
      headCount = hui.findAll('tr', head).length;
    }
    return headCount;
  },
  addRow : function(table,part) {
    if (part) {
      var trs = hui.findAll('tr', hui.find(part, table));
    } else {
      var trs = hui.findAll('tr', table);
    }
    if (trs.length>0) {
      ...
      var body = hui.find('tbody', table) || table;
